...
public class WeeklyReportViewModel : ObservableObject
{
    ...
    private async Task ExportAsync()
    {
        ...
        try
        {
            var thisWeek = ThisWeek.ToList();
            var nextWeek = NextWeek.ToList();
            await ExportHelper.ExportWithDialogAsync(ReportExporter.WeeklyFileName(Year, Week),
                path => ReportExporter.ExportWeekly(Meta, thisWeek, nextWeek, path));
        }
    ...
    {
        if (MessageBox.Show($"确定清空 {Year}-W{Week:D2} 周报的全部填写内容？", "清空表单",
